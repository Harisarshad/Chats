...
namespace Modules\Chats\Src\Controllers;// New namespace reflecting the root structure
...
class GroupController extends Controller
{
    public function store(Request $request)
    {
        ...
        return [
            'groupdata' => view('Chats::layouts.chat-group-list')->with(['groupdata' => $groupdata])->render()
        ];
    }
    ...
    public function getGroupMessage($group_id)
    {
        ...
        return [
            'view1' => view('Chats::layouts.group-message-data')->with(['messages' => $messages])->with(['groupdata' => $groupdata])->with(['userdata' => $userdata])->render(),
            'view2' => view('Chats::layouts.group-profile-details')->with(['messages' => $messages])->with(['groupdata' => $groupdata])->with(['userdata' => $userdata])->render()
        ];
    }
    ...
    public function getGroupLastMessage($group_id)
    {
        ...
        // Get all Group User data
        $userdata = DB::table('chat_group_users')->join('users', 'chat_group_users.user_id', '=', 'users.id')->select('chat_group_users.user_id', 'users.id', 'users.name', 'users.avatar')
            ->where('chat_group_users.group_id', '=', $group_id)->get();
        return view('Chats::layouts.group-conversation')->with(['messages' => $messages])->with(['userdata' => $userdata]);
    }
    ...
    public function sendGroupMessage(Request $request)
    {
        ...
        DB::table('chat_group_users')
            ->where('chat_group_users.group_id', '=', $group_id)
            ->where('chat_group_users.user_id', '!=', $from_user_id)
            ->update(['chat_group_users.is_read' => DB::raw('chat_group_users.is_read + 1')]);
        ...
        $userdata = DB::table('chat_group_users')->join('users', 'chat_group_users.user_id', '=', 'users.id')
            ->select('users.id')
            ->where('chat_group_users.group_id', '=', $group_id)->get();
        $data = ['from_user_id' => $from_user_id, 'chat_group_users' => $userdata, 'group_id' => $group_id]; // sending from and to user id when pressed enter
    ...
    public function groupsearch(Request $request)
    {
        if ($request->ajax()) { 
            ...
            $htmldata = view('Chats::layouts.chat-group-list')->with('groupdata', $groupdata)->render();
    ...
    public function groupmessagesearch(Request $request)
    {
        if ($request->ajax()) {
            ...
            $userdata = DB::table('chat_group_users')->join('users', 'chat_group_users.user_id', '=', 'users.id')
                ->select('chat_group_users.user_id', 'users.id', 'users.name', 'users.avatar')
                ->where('chat_group_users.group_id', '=', $request->groupid)->get();

            $htmldata = view('Chats::layouts.group-conversation')->with(['messages' => $messages])->with(['userdata' => $userdata])->render();
            return Response($htmldata);
        }
    }
}
